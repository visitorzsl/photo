...
#define LINE_CHART_TEXT_HEIGHT 20
...
- (void)initialization {
    ...
    _cachedMinHeight = kChartViewUndefinedCachedHeight;
                                                             //LINE_CHART_LEFT_PADDING      width - 2 * LINE_CHART_TOP_PADDING
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, width, CGRectGetHeight(self.bounds))];
...
#pragma mark - Draw Coordinate  y坐标
      ...
      //  CAShapeLayer *pointLayer = [[CAShapeLayer alloc] init];
        if ([self.delegate respondsToSelector:@selector(lineChartView:lineColorWithLineNumber:)]) {
          ...
          //  pointLayer.strokeColor = color;
           // pointLayer.fillColor = [UIColor whiteColor].CGColor;//圆圈的填充颜色
        ...
        for (NSInteger index = 0; index < array.count; index ++) {
            ...
            */
            ...
            if (lineNumber == 0 && [self.delegate respondsToSelector:@selector(lineChartView:titleAtLineNumber:)]) {
                UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(xOffset - _dotPadding/2 + 4, _chartHeight + LINE_CHART_TOP_PADDING, _dotPadding, LINE_CHART_TEXT_HEIGHT)];
